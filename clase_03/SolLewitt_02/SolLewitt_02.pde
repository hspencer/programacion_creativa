/*
 *    Straight lines 12 inches long, not touching, covering the wall evenly.
 *
 *                                                         Sol LeWitt
 */
...
void draw() {
  Point a = new Point(); 
  Point b = createPointb(a);
  if (checkIntersections(a, b) == false) {
    Line l = new Line(a, b);
    Lines.add(l);
    count++;
    println("línea número "+count+" ha sido agregada");
    l.draw();
  }
}

void keyPressed() {
  String txt = "Straight lines 12 inches long, not touching, covering the wall evenly.";
